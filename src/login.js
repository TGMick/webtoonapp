...
export default class Webread extends Component {
    constructor() {
        super();
        this.state  = {
            icon: "eye-slash",
    ...
    checkAuthorized = async () => {
        try {
            ...
            await this.props.navigate(hasKey ? 'RecommendationScreen' : 'LoginScreen');
        } catch (error) {
            console.log(error);
        }
    }
    ...
    handleButtonFunction = () => {
        axios({
            method: 'POST',
            url: 'http://192.168.1.30:5000/api/v1/login', //REST API
            data: {
            ...
            this.setState({
                token: (res.data.code === 'ERR_INCORRECT_EMAIL_PASSWORD' ? null : res.data.token),
                id: (res.data.code === "ERR_INCORRECT_EMAIL" ? null : res.data.user.id)
            })
            if (this.state.token) {
                const user = {
                ...
                storeAuthKey(user);
                this.props.navigate('RecommendationScreen').then(r => {}) //Go to Recommendation Screen
            } else {
